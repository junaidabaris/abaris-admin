...
function AddVtechStockac() {
    ...
    const sendData = async () => {
        const mapesData = showData.map((item) => {
            return { product_id: item?.productId._id, variant_id: item?.variantId._id, sku: item?.sku, stockId : item.serial_id }
        })

        const obj = { ...state, products: mapesData }

        try {
            const res = await axios.post(`https://onlineparttimejobs.in/api/serviceStockTransfer/add_TransferStock`, obj)
    ...
    const SaveData = async (val) => {
        ...
        const sendArr = arr.map((item) => {
            return { productId: item.productId, variantId: item.variantId, sku: item.variantId.sku, qty: item.qty, serialNo: item.serialNo }
        })

        try {
            const res = await axios.post(`https://onlineparttimejobs.in/api/serviceProductStock/addToCart`, { products: sendArr })
    ...
    const setTableItem = async (item) => {
        try {
            const res = await axios.get(`https://onlineparttimejobs.in/api/serviceProductStock/${item._id + '&' + state.FrompickupPoints}`)
            setcartData1(res.data);
            setModalShow(true)
            setShow(false)
        } catch (error) {
            alert('Select Pickup From!!')
        }
            ...
            <div className="row">
                ...
                <div className="card">
                    <div className="card-header d-block">
                        <h5 className="fw-600 mb-0">Add V Stock Transfer</h5>
                                                ...
                                                <table className="table table-bordered physical_product_show">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <td><label className="control-label">Serial number</label></td>
                                                        </tr>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        {showData && showData.map((item, i) => {
                                                            return <GenerateStockV item={item} i={i} showData={showData} setShowData={setShowData} DeleteRow={DeleteRow} />
                                                        })}
...
export default AddVtechStockac 