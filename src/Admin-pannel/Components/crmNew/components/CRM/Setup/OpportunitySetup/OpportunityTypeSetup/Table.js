...
const TableComponent = () => {
  ...
  const { list: opportunityTypeList, loading } = useSelector(
  ...
  const deleteHandler = (id) => {
    dispatch(deleteOpportunityType(id));
  };
  ...
  return (
    <Card className='border mt-2'>
      <div className='flex justify-between items-center'>
        ...
        <div className='flex items-center gap-2'>
          <span className='border px-2 py-1 rounded cursor-pointer bg-black text-white'>
            <CSVLink
              data={opportunityTypeList}
              className='btn btn-dark btn-sm'
              style={{ marginTop: "5px" }}
              filename='Opportunity-Type_CRM-OS'
            >
              Download CSV
            </CSVLink>
          </span>
      ...
      <Table
        ...
        dataSource={opportunityTypeList?.map((item) => ({
          ...item,
          key: item.id,
        }))}
        pagination={false}
