...
const columns = [
  {
    ...
    render: ({ attachmentName, attachmentPath }) =>
      attachmentName ? (
        <Link
          target='_blank'
          to={`${process.env.REACT_APP_API}files/${attachmentPath}`}
        >
          {attachmentName}
        </Link>
      ) : (
...

  {
    ...
    render: ({ attachmentPath }) => (
      <div className='flex justify-start'>
        <ViewBtn
          target={"_blank"}
          path={`${process.env.REACT_APP_API}files/${attachmentPath}`}
        />
      </div>
    ),
  },
...
const Attachment = () => {
  ...
  return (
    <div className='container'>
      <div className='bg-white flex flex-col md:flex-row justify-between items-center gap-3 p-3 px-6 border'>
        <div className='flex justify-end items-center gap-5'>
          <Button
            onClick={filterToggle}
            className={isFilter && "hidden md:block"}
          >
            {isFilter ? (
