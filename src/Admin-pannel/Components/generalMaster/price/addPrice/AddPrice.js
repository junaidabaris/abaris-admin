...
import AddPriceMultiLingual from "./AddPriceMultiLingual";
...
function AddPrice() {
    ...
    useEffect(() => {
        if (data) {
            const maped = data.map((item) => {
                return { name: "", min: '', max: '', description: '', slug: '', meta_title: '', meta_keyword: '', meta_description: '', sort_no: '', language_id: item._id, isActive: false, lable: item.name }
            })
            setVal(maped)
        }
    }, [data]);
    ...
    const toastErrorMessage1 = () => {
        toast.error("Price Range Not Updated ", {
            position: "top-center"
        })
    };
    ...
    const toastErrorMessage2 = () => {
        toast.error("Price Range Not Added", {
            position: "top-center"
        })
    };

    const getByIdData = async () => {
        const res = await axios.get(`https://onlineparttimejobs.in/api/priceRange/${params?.uid}`, {
            headers: {
                "Content-type": "application/json; charset=UTF-8",
                Authorization: `Bearer ${token}`,
            },
    ...
    const sendData = async () => {
        if (params?.uid) {
            try {
                const res = await axios.put(`https://onlineparttimejobs.in/api/priceRange/update_pricerange/${params?.uid}`, finalSendingD, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
            ...
            try {
                const res = await axios.post(`https://onlineparttimejobs.in/api/priceRange/add_priceRange`, finalSendingD, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
                ...
                <TabContext value={value}>
                    ...
                    {val && val.map((item, i) => {
                        return <TabPanel value={i}>
                            <div className="card">
                                <AddPriceMultiLingual setValue={setValue} data={val} params={params} item={item} i={i} sendData={sendData} onChangeHandler={onChangeHandler} onChangeThumbnailImage={onChangeThumbnailImage} showImageD={showImageD} setShowImageD={setShowImageD} />
                            </div>
