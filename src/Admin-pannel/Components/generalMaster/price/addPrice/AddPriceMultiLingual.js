...
function AddPriceMultiLingual({ data, item, i, params, sendData, onChangeHandler, setValue }) {
    return (
        <>
                    ...
                    <div className="aiz-titlebar text-left mt-2 mb-3">
                        <div className="row align-items-center">
                            <div className="col-md-6">
                                <h1 className="h3">{params?.uid ? 'Update' : 'Add'} Price</h1>
                            </div>
                        </div>
                    </div>
                    <div className="card">
                        <div className="card-header">
                            <h5 className="mb-0 h6">{params?.uid ? 'Update' : 'Add'} Price({item.lable})</h5>
                        </div>
                        <div className="card-body">
                            ...
                            <section className="form-section">
                                <div className="row">
                                    ...
                                    <div className="col-lg-4">
                                        <div>
                                            <input type="text" name="min" value={item.min} className="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="MIN*" onChange={(e) => { onChangeHandler(e, item.language_id) }} />
                                        </div>
                                    </div>
                                    <div className="col-lg-4">
                                        <div>
                                            <input type="text" name="max" value={item.max} className="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="MAX*" onChange={(e) => { onChangeHandler(e, item.language_id) }} />
                                        </div>
                                    </div>
...
export default AddPriceMultiLingual