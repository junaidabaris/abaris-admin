...
function OfflinePerchase(props) {
    ...
    const sendData = async () => {
        ...
        formData.append('transType', state.icic ? "bank" : "cheque");
        formData.append('transactionId', values.transactionId);
        formData.append('image', file);

    
        try {
            const res = await axios.post(`https://onlineparttimejobs.in/api/packagePaymentHistory/add_PackagePayment`, formData)
