...
import AddDiamondShapeMultiLingual from "./AddDiamondShapeMultiLingual";

function AddDiamondShape() {
    ...
    useEffect(() => {
        if (data) {
            const maped = data.map((item) => {
                return { name: "", code: '', slug: '', meta_title: '', meta_keyword: '', meta_description: '', sort_no: '', description: '', language_id: item._id, isActive: false, lable: item.name }
            })
            setVal(maped)
        }
    }, [data]);
    ...
    const toastErrorMessage1 = () => {
        toast.error("Diamond Shape Not Updated ", {
            position: "top-center"
        })
    };
    ...
    const toastErrorMessage2 = () => {
        toast.error("Diamond Shape Not Added", {
            position: "top-center"
        })
    };

    const getByIdData = async () => {
        const res = await axios.get(`https://onlineparttimejobs.in/api/shape/${params?.uid}`, {
            headers: {
                "Content-type": "application/json; charset=UTF-8",
                Authorization: `Bearer ${token}`,
            },
    ...
    const sendData = async () => {
        if (params?.uid) {
            try {
                const res = await axios.put(`https://onlineparttimejobs.in/api/shape/update_shape/${params?.uid}`, finalSendingD, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
            ...
            try {
                const res = await axios.post(`https://onlineparttimejobs.in/api/shape/add_shape`, finalSendingD, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
                ...
                <TabContext value={value}>
                    ...
                    {val && val.map((item, i) => {
                        return <TabPanel value={i}>
                            <div className="card">
                                <AddDiamondShapeMultiLingual setValue={setValue} data={val} params={params} item={item} i={i} sendData={sendData} onChangeHandler={onChangeHandler} onChangeThumbnailImage={onChangeThumbnailImage} showImageD={showImageD} setShowImageD={setShowImageD} />
                            </div>
