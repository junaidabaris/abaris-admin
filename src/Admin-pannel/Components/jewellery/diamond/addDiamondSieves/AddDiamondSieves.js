...
import AddDiamondSievesMultiLingual from "./AddDiamondSievesMultiLingual";
...
function AddDiamondSieves() {
    ...
    const toastSuccessMessage1 = () => {
        toast.success("Diamond Sieves Updated", {
            position: "top-center"
        })
    };
    const toastErrorMessage1 = () => {
        toast.error("Diamond Sieves Not Updated ", {
            position: "top-center"
        })
    };

    const toastSuccessMessage2 = () => {
        toast.success("Diamond Sieves Added", {
            position: "top-center"
        })
    };
    const toastErrorMessage2 = () => {
        toast.error("Diamond Sieves Not Added", {
            position: "top-center"
        })
    };
    ...
    const getByIdData = async () => {
        const res = await axios.get(`https://onlineparttimejobs.in/api/sieves/${params?.uid}`, {
            headers: {
                "Content-type": "application/json; charset=UTF-8",
                Authorization: `Bearer ${token}`,
            },
    ...
    const sendData = async () => {
        if (params?.uid) {
            try {
                const res = await axios.put(`https://onlineparttimejobs.in/api/sieves/update_sieves/${params?.uid}`, finalSendingD, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
            ...
            try {
                const res = await axios.post(`https://onlineparttimejobs.in/api/sieves/add_sieves`, finalSendingD, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
                ...
                <TabContext value={value}>
                    ...
                    {val && val.map((item, i) => {
                        return <TabPanel value={i}>
                            <div className="card">
                                <AddDiamondSievesMultiLingual setValue={setValue} data={val} params={params} item={item} i={i} sendData={sendData} onChangeHandler={onChangeHandler} onChangeThumbnailImage={onChangeThumbnailImage} showImageD={showImageD} setShowImageD={setShowImageD} />
                            </div>
